/* Les vues pour la foire */

CREATE OR REPLACE FUNCTION public.prioritaire(anciennete real) RETURNS boolean AS $$
    BEGIN
        RETURN anciennete > 0 AND anciennete < 4;
    END; 
$$ LANGUAGE plpgsql ;
...
/* il faut sans doute revoir ces vues pour qu'elles soient plus efficaces */

CREATE OR REPLACE VIEW voeu_enseignement_detail_temp_prioritaire AS
SELECT e.id,
    sum(v.cm) AS cm,
    sum(v.ctd) AS ctd,
    sum(v.td) AS td,
   ...
   FROM enseignement e
     LEFT JOIN voeu v ON v.enseignement = e.id
  WHERE (v.id > 0 OR v.id IS NULL) AND v.actif AND prioritaire(v.anciennete)
  GROUP BY e.id;


CREATE OR REPLACE VIEW voeu_enseignement_detail_prioritaire AS
SELECT e.id,
...
CREATE OR REPLACE VIEW voeu_enseignement_bilan_prioritaire AS
 SELECT e.id,
